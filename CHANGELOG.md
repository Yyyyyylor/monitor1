...
- WebUI 合并重叠的轮询、实时推送与操作请求；页面切换会取消过期请求，库存主体优先呈现，变化记录与归档独立加载。
- 监控调度改为有界 worker 队列；批量导入预取已有记录，减少重复数据库查询；Steam 429 的 `Retry-After` 等待受可配置上限保护。

### Fixed

- 补齐页面请求、归档和快照比较的错误状态与重试入口，避免慢网、空库存或已删除快照导致过期页面覆盖或未处理异步错误。
- 服务停止与启动失败时有序回收监控 worker、归档任务、WebSocket、HTTP 客户端和数据库，避免后台任务残留或资源关闭竞态。
